<span>
    <span class="flex max-w-2xl gap-4 overflow-auto not-prose sm:px-0 snap-x snap-mandatory">
       ...
       <span class="w-48 rounded-lg snap-center shrink-0 focus:ring-2 ring-red-500">
            <img 
            @if($enable_modal) onclick="window.livewire.emit('showImageModal','{{ asset('storage/' . $image['path']) }}', '{{ $image['title'] }}')" @endif class="cursor-pointer relative object-cover w-full h-full rounded-lg" src="{{ asset('storage/' . $image["path"]) }}"
